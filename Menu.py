...
class Menu(SetupWindow):
    ...
    def rules_loop(self, file):
        while not self.crashed:
            ...
            self.graphics.message_display('Zasady:', 50, self.displayWidth/2, self.displayHeight/20, colors["SUMMER_SUN"])
            i = 80
            with open(file, 'r') as text:
                ...
                for line in lines:
                    ...
                    if line != '\n':
                        self.graphics.message_display(line, 12, self.displayWidth/2, i, colors["SUMMER_SUN"])
                        i += 20
        ...
        while not self.crashed:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self.terminate_game()
            self.screen.fill(colors["NIGHT_OF_NAVY"])
            self.graphics.message_display('Warcaby', 80, self.displayWidth/2, self.displayHeight/10, colors["SUMMER_SUN"])
    ...
    def gameLoop(self):
        while self.game.backMenu is False:
            self.screen.fill(colors["NIGHT_OF_NAVY"])
            self.game.eventLoopPvP()
        self.game = Game()
